...
then
       echo "\nUsage:"
        ...
        echo "Example:"
        echo "-------"
        echo "       RefreshPackage -u ccuser -p **** -e \"Tasks 750\" -s \"Build Approval\"\n"
        exit
fi
...
if [ "$env_name" = "" ]
then
	echo "\nPlease write the Harvest Enviroment name (e.g. Tasks 750):"
	read env_name
fi
...
log_dir=$HOME/log/ccRefreshPackage
mkdir -p $HOME/log/ccRefreshPackage
...
echo "Start processing information regarding files to Refresh ...\n" | tee -a $refresh_log
...
for rep_dir in `cat $tmp_dir_list`
{
