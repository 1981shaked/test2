...
#############################    Check Argument    ##################################
	...
	then
		print "\nUsage  : ccIcmBuild < Version Number > [ Line ] [ -m < Email Address > ] [ -SM ]\n"
		print "[ Line ] : Number of line to start Icm build , Line 1 is the defult"
...
function SentMail {
	...
	echo "\nUMB link : http://indibm016:58808/" >> $Mail_Log
	echo "\nThanks : CC team\n" >> $Mail_Log
	ICM_BUILD_NUM=`echo $Icm_Build_Num`
...
if [[ $Start_LineNum -gt $Line_Num ]]
then
        print "\nError : Start line is greater than line number\n"
        exit
fi
