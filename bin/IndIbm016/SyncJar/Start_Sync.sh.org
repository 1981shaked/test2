...
#-----------------------------
# echo -- set build counter --
#-----------------------------

### may need customized for every project according to the project build number retriving
#if (  $Build_Number == 1 ) then
#        set Build_Number=`buildCounter Daily $Ver 0 | awk '{print $4}'`
#        if ( $? != 0 ) then
#        	echo "Can not get build number for version $Ver  "
#        	exit (1)
#        endif        
#endif
...
foreach line ( `cat $SyncPropertyFile ` )
 ...
 # the above chmod command will result error msg (chmod: can't access...) when files do not exist - 
 # this err msg can be ignored
 ...
 #---------------------------------------
 # copy file to tmp target directory
 #---------------------------------------
 ...
 #--------------------------------------------
 #  make sure jars exists in source directory
 #--------------------------------------------

 if ( ! -d ${Src_tmp_dir} ) then 
        echo " ERROR:  ${Src_tmp_dir} Directory  does not exist" 
 ...
 #------------------------------------------------------------------------------
 # Create/Set  Task Package 
 #------------------------------------------------------------------------------

 cd ${Src_tmp_dir}

 # Append info line per task for later on activities
 #----------------------------------------------------
 ...
 # set warning flag per Task if a file had a copy warning
 if ("${Warning_flag_Local}" == 1 )then
    rm -fr $Task_List_TMP
 ...
 hco -r -op as -up -en "${Task_Ver}" -b $Broker_name $Usr_Pass -st "${Dev_State}" -vp "/$View_Path" -pn "$CO_Process_Name" -p "$Task_name" $File_name
 if ( $? == 0 ) then
   	echo "SUCCESS: checkout" >> ${Full_pair_log}
 else
	echo "ERROR: checkout "
	less hco.log >> ${Full_pair_log}
	rm -fr $Task_List_TMP
    	sed '/^'${Task_name}'/s/error_flag_0/error_flag_1/g' ${Task_List} > !  $Task_List_TMP
    	mv -f $Task_List_TMP $Task_List
 endif
 ...
 hci -nd -if ne -en "${Task_Ver}" -b $Broker_name $Usr_Pass -st "${Dev_State}" -vp "/$View_Path" -pn "$CI_Process_Name" -p "$Task_name" -de "Build $Build_Number" $File_name
 if ( $? == 0 ) then
    	echo "SUCCESS: check in "  >> ${Full_pair_log}
 else
    	echo "ERROR: check in "
    	less hci.log >> ${Full_pair_log}
	rm -fr $Task_List_TMP
	sed '/^'${Task_name}'/s/error_flag_0/error_flag_1/g' ${Task_List} > ! $Task_List_TMP
    	mv -f $Task_List_TMP $Task_List
