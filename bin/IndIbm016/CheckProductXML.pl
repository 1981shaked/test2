...
if ( $product =~ /amss/i){	
	@project_list = (c3ecrbe.$version_num.V.$variant,c9ecrbe.$version_num.V.$variant,ecrgdd.$version_num.V.$variant);
	print "This is a project $project_list[0] \n";
}
...
sub run_check_for_project {
	...
	foreach (<PROJ_PROFILE>) {
		...
		($bb_name,$bb_ver) = (split(/\s+/,$_))[0,1] ;
			...
			elsif ($pname =~ /c[3579]ecrbe/){
				if ( $bb_name =~ /cbe[3579]_deploy/ ){
					print "$bb_name bb of ecare\n";
					&find_main_list ;
			...
			elsif ($pname =~ /c[3579]rm$version_num/){
				if ( $bb_name =~ /crm/ ){
					&check_ant_failure;
				}	
				else {		
					...
					&check_timeout_failure;
					&check_ant_failure;
					&check_generator_failure;					
				}
			}

			elsif ($pname =~ /c[3579]se$version_num/){
				if ( $bb_name =~ /cse/ ){
					&check_ant_failure;
				}		
				else {				
			...
			elsif ($pname =~ /c[3579]aps$version_num/){
				if ( $bb_name =~ /aps/ ){
					&check_ant_failure;
				}		
			}			
  	...
  	foreach $jar_command ( @jartarget ){
  		...
  		if (grep (/\.jar$/, $jar)){ #if file finished with .jar
   			if (grep (/\$\{env\.ACCOUNT_NAME\}/, $jar)){
					print "$ENV{ACCOUNT_NAME} ---- build_${bb_name}.xml \n";
  			...
  			elsif (grep (/\$\{env\.ACCOUNT_NAME9\}/, $jar)){

  				$jar = (split(/\$\{env\.ACCOUNT_NAME9\}/,$jar))[1];
  				
  				if (grep (/\$\{env\.CCBB\}/, $jar)){
  					$jar = $ENV{ACCOUNT_NAME9}.$bb_name.".jar";
  				}
  				else {
  					$jar = $ENV{ACCOUNT_NAME9}.$jar;
				  }
...
sub check_oms_server_compilation {
  ...
  open(LOG_FILE,"$build_log_dir/$log_file") || warn "Can't open $log_file"; 	
 	if (grep/Error executing command/,<LOG_FILE>){
 		push (@server_errors,$bb_name) ;
		push (@proj_server_errors,$bb_name) ;
		$bb_failure_status = 1 ;
		$proj_total_server_err = 1;
...
sub oms_utilities_build{
	...
	if ( $bb_name =~ m/cord9utilities/ ){
  	...
  	open(LOG_FILE,"$build_log_dir/$log_file") || warn "Can't open $log_file" ;
		...
		if ( grep/java\.lang\.StackOverflowError/,<LOG_FILE> ){ 
			push (@server_errors,$bb_name) ;
			push (@proj_server_errors,$bb_name) ;
			$bb_failure_status = 1 ;
			$proj_total_server_err = 1;
...
foreach $current_topic ( @dynamic_topics ) {
  ...
  next if (&check_excluded("$current_topic") || (($bb_name =~ /cabm9som/)&&( $current_topic =~ /junit/)) || (($bb_name =~ /cabm9som/)&&( $current_topic =~ /simulator/))) ;  
  ...
  foreach $java_file (readdir (TOPIC)) {
      ...
      $manipulated_topic[0] =~ tr/\./\//;
			...
			@com_topic = split (/\/com\//,$current_topic);
			$com_topic = "com/".$com_topic[1];
			
			if (! `grep package ${src_area}/$current_topic/$java_file`){
      	print "hello ${src_area}/$current_topic/$java_file\n";
      	$com_topic = "";
      }
      ...
      if (! -f "$proj_area/classes/$current_topic/$class_file" && ! -f "$proj_area/ut/classes/${bb_name}/$current_topic/$class_file" && ! -f "$proj_area/${bb_name}/classes/$current_topic/$class_file" && ! -f "$proj_area/${bb_name}/classes/$manipulated_topic[0]/$class_file" && ! -f "$proj_area/${bb_name}/classes/$java_topic/$class_file" && ! -f "$proj_area/classes/$java_topic/$class_file" && ! -f "$proj_area/classes/$manipulated_topic[0]/$class_file" && ! -f "$proj_area/$bb_name/AifJavaClasses/$manipulated_topic[0]/$class_file" && ! -f "$proj_area/classes/$com_topic/$class_file" && ! -f "$proj_area/${bb_name}/webservices/UserProvisioning/classes/$com_topic/$class_file" && ! -f "$proj_area/${bb_name}/autoResponse/classes/$com_topic/$class_file" && ! -f 
      "$proj_area/${bb_name}/wizards/project-te/classes/$com_topic/$class_file" && ! -f "$proj_area/${bb_name}/wizards/undo-tasks/classes/$com_topic/$class_file" && ! -f "$proj_area/${bb_name}/dbserver/api_java/classes/$com_topic/$class_file" && ! -f
      "$proj_area/${bb_name}/wizards/process-tasks/classes/$com_topic/$class_file" ) {
...
######################################################
## sub get_last_log - function retirns last log - 
## which name is starting from hbuild.log or build
######################################################
...
sub get_last_log {
  ...
  while(<LOG_DIR>){
	...
	print "The log files from function is $log_file \n";
	return $log_file;
...
#$build_log_dir = "$ENV{HOME}/log.$pname/log.$bb_name" ;
	...
	print "The log file for bb $bb_name - $log_file \n";
  ...
  if ( grep(/BUILD FAILED/,<LOG_FILE>)) {
	...
	open(LOG_FILE,"$build_log_dir/$log_file") || warn "Can't open $log_file" ;
  if ( grep(/java\.lang\.NoClassDefFoundError/,<LOG_FILE>)) {
     push (@ANT_errors,$bb_name) ;
     push (@proj_ANT_errors,$bb_name) ;
     $bb_failure_status = 1 ;
  }
  ...
  open(LOG_FILE,"$build_log_dir/$log_file") || warn "Can't open $log_file" ;
  if ( grep(/Schema Combine ended with exit status/,<LOG_FILE>)) {
     push (@ANT_errors,$bb_name) ;
     push (@proj_ANT_errors,$bb_name) ;
     $bb_failure_status = 1 ;
  }
  ...
  open(LOG_FILE,"$build_log_dir/$log_file") || warn "Can't open $log_file" ;
  if ( grep(/Java Result: 1/,<LOG_FILE>)) {
     push (@ANT_errors,$bb_name) ;
     push (@proj_ANT_errors,$bb_name) ;
     $bb_failure_status = 1 ;
  }
  ...
  open(LOG_FILE,"$build_log_dir/$log_file") || warn "Can't open $log_file" ;
  if ( grep(/failure in processing DataMap/,<LOG_FILE>)) {
     push (@ANT_errors,$bb_name) ;
     push (@proj_ANT_errors,$bb_name) ;
     $bb_failure_status = 1 ;
  }
	...
	open(LOG_FILE,"$build_log_dir/$log_file") || warn "Can't open $log_file" ;
  if ( grep(/java.lang.NullPointerException/,<LOG_FILE>)) {
     push (@ANT_errors,$bb_name) ;
     push (@proj_ANT_errors,$bb_name) ;
     $bb_failure_status = 1 ;
  }
...
sub check_generator_failure {
  ...
  if ( grep(/Error invoking generator/,<LOG_FILE>) ) {
     push (@GEN_errors,$bb_name) ;
     push (@proj_GEN_errors,$bb_name) ;
     $bb_failure_status = 1 ;
  }
...
sub print_errors {
	...
	foreach (@GEN_errors) {
 		LogWrite ($FULL_LOG,"  <error name='ERROR: Generator problems with $_ .'> </error> ") ;
	}
...
foreach (@proj_GEN_errors) {
  &print_with_xml_format($pname,"Generator failure","problems with ant","($_)") ;
}
...
sub send_email {
	...
	$subject = "The Build $product_build_counter for $product version $version $variant variant on $host" ;
